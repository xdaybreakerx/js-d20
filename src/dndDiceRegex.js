 ...
 * @author Xander
 *
 * @param {*} command A function that parses and executes a dice command
 * @returns {*} Parses and executes a dice command provided as input, following the specified regex pattern. Calculates the total roll based on the parsed components including the number of dice, type of dice, optional modifiers like 'keep highest/lowest', and arithmetic modifiers.
 */
function roll(diceString) {
  ...
  */
  ...
  const keepHighestOrLowest = matches[3] ? matches[3].substring(0, 2) : null;
  const keepNumberOfDice = parseInt(matches[4]) || 0;
  const rollModifier = parseInt(matches[6]) || 0;

  // Roll the dice based on the parsed command
  let rolledDice = rollMultipleDice(numberOfDice, numberOfSides);

  // Sort and possibly reduce the number of dice based on keep preferences
  let relevantRolls = sortDiceRolls(rolledDice, keepHighestOrLowest, keepNumberOfDice);

  // Calculate the total of the relevant dice rolls including any modifiers
  let finalResult = calculateDiceRollTotal(relevantRolls, rollModifier);

  // Log the detailed results for debugging and verification
  console.log(`Dice Command: ${diceString}, Rolls: ${rolledDice.join(", ")}, Kept: ${relevantRolls.join(", ")}, Final Total: ${finalResult}`);
  return finalResult;

  console.log(`Rolls: ${rolls.join(", ")}, Modifier: ${modifier}, Total: ${total}`);
  return total;
